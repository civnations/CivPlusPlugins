        ...
        if (entity.bA()) {
            if (!entity.t(attacker)) {
                ...
                if (f > 0.0F || f1 > 0.0F) {
                    ...
                    f += f1;
                    boolean flag3 = false;
                    double d0 = (double) (attacker.H - attacker.G);
					
                    if (!flag2 && !attacker.isSprinting() && attacker.onGround && d0 < (double) attacker.dt()) {
                    ...
                    if (flag5) {
						...
						// Adds x, y, z to the entity's current velocity (basically)
                    	entity.h(x, y, z);
